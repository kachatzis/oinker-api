...
exports.getEmailServices = async (req, reply) => { 
  try {
    ...
    const emailService = await EmailService.find({user : oauth.user})
    return emailService
  } catch (err) {
...
exports.getSingleEmailService = async (req, reply) => {
  try {
    ...
    const emailService = await EmailService.findOne({"_id": id, "user": oauth.user})
    return emailService
  } catch (err) {
    throw boom.boomify(err)
  }
...
exports.updateEmailService = async (req, reply) => {
  try {
    ...
    const update = await EmailService.findOneAndUpdate({"_id":id, "user":oauth.user}, updateData, { new: true })
    return update
  } catch (err) {
    throw boom.boomify(err)
  }
